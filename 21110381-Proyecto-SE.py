...
root.config(bg="#4A376D")  # Color de fondo oscuro relacionado a tipo fantasma

# Preguntas y entradas con colores morados
tk.Label(root, text="Tipo de Pokémon enemigo (fuego, agua, etc.):", bg="#4A376D", fg="white", font=("Arial", 12)).pack(pady=5)
tipo_enemigo_var = tk.StringVar()
tk.Entry(root, textvariable=tipo_enemigo_var, bg="#5A497E", fg="white", font=("Arial", 10)).pack(pady=5)

tk.Label(root, text="Preferencia de tipo de ataque (físico/especial):", bg="#4A376D", fg="white", font=("Arial", 12)).pack(pady=5)
tipo_preferencia_var = tk.StringVar()
tk.Entry(root, textvariable=tipo_preferencia_var, bg="#5A497E", fg="white", font=("Arial", 10)).pack(pady=5)

tk.Label(root, text="Nivel preferido (bajo/medio/alto):", bg="#4A376D", fg="white", font=("Arial", 12)).pack(pady=5)
nivel_preferencia_var = tk.StringVar()
tk.Entry(root, textvariable=nivel_preferencia_var, bg="#5A497E", fg="white", font=("Arial", 10)).pack(pady=5)

tk.Label(root, text="Velocidad preferida (alta/media/baja):", bg="#4A376D", fg="white", font=("Arial", 12)).pack(pady=5)
velocidad_preferencia_var = tk.StringVar()
tk.Entry(root, textvariable=velocidad_preferencia_var, bg="#5A497E", fg="white", font=("Arial", 10)).pack(pady=5)

# Botón para obtener recomendaciones con color morado oscuro
tk.Button(root, text="Obtener Recomendaciones", command=mostrar_recomendaciones, bg="#8968AC", fg="white", font=("Arial", 12)).pack(pady=20)
