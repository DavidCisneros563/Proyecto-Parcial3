...
def mostrar_recomendaciones():
    ...
    if not recomendaciones:
        ...
        resultado = "\n".join([f"{nombre} ({tipo}) - Puntos: {puntos}" for nombre, tipo, puntos in recomendaciones])
...
root.config(bg="#2E2E38")  # Color de fondo oscuro relacionado a tipo fantasma

# Preguntas y entradas con colores morados
tk.Label(root, text="Tipo de Pokémon enemigo (fuego, agua, etc.):", bg="#2E2E38", fg="white", font=("Arial", 12)).pack(pady=5)
tipo_enemigo_var = tk.StringVar()
tk.Entry(root, textvariable=tipo_enemigo_var, bg="#3C3C4B", fg="white", font=("Arial", 10)).pack(pady=5)

tk.Label(root, text="Preferencia de tipo de ataque (físico/especial):", bg="#2E2E38", fg="white", font=("Arial", 12)).pack(pady=5)
tipo_preferencia_var = tk.StringVar()
tk.Entry(root, textvariable=tipo_preferencia_var, bg="#3C3C4B", fg="white", font=("Arial", 10)).pack(pady=5)

tk.Label(root, text="Nivel preferido (bajo/medio/alto):", bg="#2E2E38", fg="white", font=("Arial", 12)).pack(pady=5)
nivel_preferencia_var = tk.StringVar()
tk.Entry(root, textvariable=nivel_preferencia_var, bg="#3C3C4B", fg="white", font=("Arial", 10)).pack(pady=5)

tk.Label(root, text="Velocidad preferida (alta/media/baja):", bg="#2E2E38", fg="white", font=("Arial", 12)).pack(pady=5)
velocidad_preferencia_var = tk.StringVar()
tk.Entry(root, textvariable=velocidad_preferencia_var, bg="#3C3C4B", fg="white", font=("Arial", 10)).pack(pady=5)

# Botón para obtener recomendaciones con color morado oscuro
tk.Button(root, text="Obtener Recomendaciones", command=mostrar_recomendaciones, bg="#6C63FF", fg="white", font=("Arial", 12)).pack(pady=20)
